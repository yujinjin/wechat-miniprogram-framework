...
module.exports = {
  ...
  /*filter(node) {
    console.log(node);
    if (node.name === 'img' && /^http\:\/\//.test(node.attrs.src)) {
      console.log(node.attrs.src);
      node.attrs.src = node.attrs.src.replace(/^http\:\/\//, 'https://')
    }
  },*/
  // 代码高亮函数
  highlight: null,
  ...
  // 信任的属性
  trustAttrs: makeMap('align,alt,app-id,author,autoplay,border,cellpadding,cellspacing,class,color,colspan,controls,data-src,dir,face,height,href,id,ignore,loop,media,muted,name,path,poster,rowspan,size,span,src,start,style,type,unit-id,width,xmlns,data-id'),
  // bool 型的属性
